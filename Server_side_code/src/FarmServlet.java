...
protected void doGet(HttpServletRequest request,
	...
	try {
		...
		PreparedStatement ps = con
				.prepareStatement("insert into farm values(?,?,?)");
		ResultSet rs = ps.executeQuery("SELECT MAX(id) AS id FROM farm");
		if(rs.next()){
			id = rs.getInt("id");
			id++;
			}
		...
		if (i1 > 0) {
			System.out.print("Farm details inserted successfully");
			out.writeObject("Farm details inserted successfully");
		} else {
			out.writeObject("Data not get inserted.");
		}
	} catch (Exception e) {
		e.printStackTrace();
	}
